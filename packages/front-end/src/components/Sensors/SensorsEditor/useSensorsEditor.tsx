...
import { SelectChangeEvent } from "@mui/material";
  ...
  export const useSensorEditor = (isEdit: boolean, sensorToEdit?: Sensor) => {
    const [name, setName] = useState<string>(isEdit ? sensorToEdit?.name || "" : "");
    const [selectedSensorType, setSelectedSensorType] = useState<number>(isEdit ? sensorToEdit?.sensorType.id || SensorTypeList[0].id : SensorTypeList[0].id);
    const [identifier, setIdentifier] = useState<string>(isEdit ? sensorToEdit?.identifier || "" : "");
    const [selectedFrequency, setSelectedFrequency] = useState<number>(isEdit ? sensorToEdit?.frequency.id || FrequencyList[0].id : FrequencyList[0].id);
  
    const handleNameChange = (event: React.ChangeEvent<HTMLInputElement>) => {
      setName(event.target.value);
    };
  
    // const handleSensorTypeChange = (event: React.ChangeEvent<{ value: unknown }>) => {
